...
fechar as três pendências das ondas 1 e 3.
...
-- terceiro estado — a fonte nao publica nada. O Seize Jaguar e o primeiro. O
...
### Task 8: As pendências antigas

**Arquivos:**
- Modificar: `data/parts/blades.json`, `data/parts/integrated_blades.json`,
  `data/beyblades/bx.json`, `data/beyblades/ux.json` (pela coleta)
...
- [ ] **Passo 2: coletar as pendências**

```bash
npm run coletar -- --paginas "Blade - Lightning L-Drago (Upper Type);Blade - Lightning L-Drago (Rapid-Hit Type);Ratchet-Integrated Blade - Cutter Shinobi;Ratchet-Integrated Blade - Rampart Aegis;Ratchet-Integrated Blade - Valor Bison;Ratchet-Integrated Blade - Seize Jaguar"
```

Esperado: 6 peças, 0 descartes.

- [ ] **Passo 3: conferir os três casos que motivaram a tarefa**

```bash
node -e "
const c=f=>Object.values(require('./data/parts/'+f+'.json')).find(Array.isArray);
const t=[...c('blades.json'),...c('integrated_blades.json')];
const ld=t.filter(p=>/L-Drago/.test(p.name));
console.log('Lightning L-Drago:', ld.length, '| giro:', [...new Set(ld.map(p=>p.spin_direction))]);
const sj=t.find(p=>/Seize Jaguar/.test(p.name));
console.log('Seize Jaguar -> data_disputed:', sj.data_disputed);
console.log('  notes:', sj.notes);
"
```

Esperado: 2 lâminas L-Drago, giro `left` — as primeiras canhotas do catálogo.
`Seize Jaguar` com `data_disputed: true` e a nota dizendo que a fonte não
publica.
...
git commit -m "feat(catalogo): fecha as pendencias das ondas 1 e 3"
```
